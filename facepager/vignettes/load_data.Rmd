...
example_db <- fp_load_database(system.file("extdata", "exampledata.db", package = "facepager"))
# db Dateien noch nicht im extdata ordner
```
...
```{r}
data <- fp_read_csv2(system.file("extdata", "exampledata.csv", package = "facepager"))
...
```{r}
data <- fp_loaddatabase(system.file("extdata", "exampledata.db", package = "facepager"))
# db Datei noch nicht im extdata ordner
```
...
```{r}
filenames <- c(
  system.file("extdata", "exampledata.db", package = "facepager"), 
  system.file("extdata", "exampledata2.db", package = "facepager")
  )
...
```{r}
data <- fp_load_nodes(system.file("extdata", "exampledata.db", package = "facepager"), 
                      fields = 'query_type, query_time, object_type')
```
...
```{r}
# eigentlich: fp_parse_response_data(data)
# aber Fehlermeldung: 
# Error in names(x) <- value : 
#   'names' attribute [1] must be the same length as the vector [0] 
# Calls: <Anonymous> ... eval -> eval -> fp_parse_response_data -> colnames<-
data <- fp_parse_response_data(example)
```
